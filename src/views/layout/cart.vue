    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="cart-list">
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          ...
          ></van-checkbox>
          <div class="show" @click="$router.push(`/goods/${item.goods_id}`)">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          <div class="price">
            <span>合计：</span>
            <span
              >¥ <i class="totalPrice">{{ selectPrice }}</i></span
            >
          </div>
          <div
            @click="goPay"
...
export default {
  ...
  methods: {
    ...
    toggleAllCheck () {
      this.$store.commit('cart/toggleAllCheck', !this.allCheck)
    },
    ...
    goPay () {
      ...
      if (this.selectCount > 0) {
        ...
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selectCart.map(item => item.id).join(',')
          }
        })
      }
    }
